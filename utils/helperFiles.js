...
import * as helper from './helperFunction';
...
const fileUtils = {
  ...
  async publishUnpublish(request, setPublish) {
    ...
    if (!helper.checker(fileId)) { return { error: 'Unauthorized', code: 401 }; }

    const { userId } = await helper.getIdAndKey(request);

    if (!helper.checker(userId)) { return { error: 'Unauthorized', code: 401 }; }
